...
<td>${row.Name}</td>
<td>
  <select class="option_attendance5" id="option_attendance5${index}">
...
const select = newRow.querySelector(`#option_attendance5${index}`);
select.value = localStorage.getItem(`option_attendance5${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance5");
...
localStorage.setItem(`option_attendance5${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday5');
...
localStorage.setItem('titleClassToday5', newText);
}
});
...
saveTextToLocalStorage("materialName5", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName5", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName5");
var savedTaskName = loadTextFromLocalStorage("taskName5");
...
editedData5.push({ name, score, evaluation, report });
});

localStorage.setItem('editedData5', JSON.stringify(editedData5));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData5JSON = localStorage.getItem('editedData5');
if (editedData5JSON) {
const editedData5 = JSON.parse(editedData5JSON);

const tableRows = Array.from(document.querySelectorAll('#table-score-content tr'));
tableRows.forEach((row, index) => {
row.cells[1].textContent = editedData5[index].score;
row.cells[2].textContent = editedData5[index].evaluation;
// row.cells[3].textContent = editedData5[index].report;
});
}
}
