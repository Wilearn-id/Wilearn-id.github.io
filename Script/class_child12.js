...
<td>${row.Name}</td>
<td>
  <select class="option_attendance12" id="option_attendance12${index}">
...
const select = newRow.querySelector(`#option_attendance12${index}`);
select.value = localStorage.getItem(`option_attendance12${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance12");
...
localStorage.setItem(`option_attendance12${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday12');
...
localStorage.setItem('titleClassToday12', newText);
}
});
...
saveTextToLocalStorage("materialName12", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName12", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName12");
var savedTaskName = loadTextFromLocalStorage("taskName12");
...
editedData12.push({ name, score, evaluation, report });
});

localStorage.setItem('editedData12', JSON.stringify(editedData12));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData12JSON = localStorage.getItem('editedData12');
if (editedData12JSON) {
const editedData12 = JSON.parse(editedData12JSON);

const tableRows = Array.from(document.querySelectorAll('#table-score-content tr'));
tableRows.forEach((row, index) => {
row.cells[1].textContent = editedData12[index].score;
row.cells[2].textContent = editedData12[index].evaluation;
// row.cells[3].textContent = editedData12[index].report;
});
}
}
