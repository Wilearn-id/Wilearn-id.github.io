...
<td>${row.Name}</td>
<td>
  <select class="option_attendance11" id="option_attendance11${index}">
...
const select = newRow.querySelector(`#option_attendance11${index}`);
select.value = localStorage.getItem(`option_attendance11${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance11");
...
localStorage.setItem(`option_attendance11${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday11');
...
localStorage.setItem('titleClassToday11', newText);
}
});
...
saveTextToLocalStorage("materialName11", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName11", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName11");
var savedTaskName = loadTextFromLocalStorage("taskName11");
...
editedData11.push({ name, score, evaluation, report });
});

localStorage.setItem('editedData11', JSON.stringify(editedData11));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData11JSON = localStorage.getItem('editedData11');
if (editedData11JSON) {
const editedData11 = JSON.parse(editedData11JSON);

const tableRows = Array.from(document.querySelectorAll('#table-score-content tr'));
tableRows.forEach((row, index) => {
row.cells[1].textContent = editedData11[index].score;
row.cells[2].textContent = editedData11[index].evaluation;
// row.cells[3].textContent = editedData11[index].report;
});
}
}
