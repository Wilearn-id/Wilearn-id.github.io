...
<td>${row.Name}</td>
<td>
  <select class="option_attendance7" id="option_attendance7${index}">
...
const select = newRow.querySelector(`#option_attendance7${index}`);
select.value = localStorage.getItem(`option_attendance7${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance7");
...
localStorage.setItem(`option_attendance7${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday7');
...
localStorage.setItem('titleClassToday7', newText);
}
});
...
saveTextToLocalStorage("materialName7", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName7", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName7");
var savedTaskName = loadTextFromLocalStorage("taskName7");
...
editedData7.push({ name, score, evaluation, report });
});

localStorage.setItem('editedData7', JSON.stringify(editedData7));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData7JSON = localStorage.getItem('editedData7');
if (editedData7JSON) {
const editedData7 = JSON.parse(editedData7JSON);

const tableRows = Array.from(document.querySelectorAll('#table-score-content tr'));
tableRows.forEach((row, index) => {
row.cells[1].textContent = editedData7[index].score;
row.cells[2].textContent = editedData7[index].evaluation;
// row.cells[3].textContent = editedData7[index].report;
});
}
}
