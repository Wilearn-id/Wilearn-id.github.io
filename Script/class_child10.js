...
<td>${row.Name}</td>
<td>
  <select class="option_attendance10" id="option_attendance10${index}">
...
const select = newRow.querySelector(`#option_attendance10${index}`);
select.value = localStorage.getItem(`option_attendance10${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance10");
...
localStorage.setItem(`option_attendance10${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday10');
...
localStorage.setItem('titleClassToday10', newText);
}
});
...
saveTextToLocalStorage("materialName10", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName10", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName10");
var savedTaskName = loadTextFromLocalStorage("taskName10");
...
localStorage.setItem('editedData10', JSON.stringify(editedData10));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData10JSON = localStorage.getItem('editedData10');
if (editedData10JSON) {
const editedData10 = JSON.parse(editedData10JSON);
