...
<td>${row.Name}</td>
<td>
  <select class="option_attendance8" id="option_attendance8${index}">
...
const select = newRow.querySelector(`#option_attendance8${index}`);
select.value = localStorage.getItem(`option_attendance8${index}`) || "hadir";
...
const selects = document.querySelectorAll(".option_attendance8");
...
localStorage.setItem(`option_attendance8${index}`, select.value);
});
});
...
var savedText = localStorage.getItem('titleClassToday8');
...
localStorage.setItem('titleClassToday8', newText);
}
});
...
saveTextToLocalStorage("materialName8", newMaterialName);
}
});
...
saveTextToLocalStorage("taskName8", newTaskName);
}
});

// Load the saved text from local storage when the page is loaded
window.addEventListener("load", function() {
var savedMaterialName = loadTextFromLocalStorage("materialName8");
var savedTaskName = loadTextFromLocalStorage("taskName8");
...
editedData8.push({ name, score, evaluation, report });
});

localStorage.setItem('editedData8', JSON.stringify(editedData8));
}

// Function to load the edited values from local storage
function loadEditedValuesReportFromLocalStorage() {
const editedData8JSON = localStorage.getItem('editedData8');
if (editedData8JSON) {
const editedData8 = JSON.parse(editedData8JSON);

const tableRows = Array.from(document.querySelectorAll('#table-score-content tr'));
tableRows.forEach((row, index) => {
row.cells[1].textContent = editedData8[index].score;
row.cells[2].textContent = editedData8[index].evaluation;
// row.cells[3].textContent = editedData8[index].report;
});
}
}
